...
opendiscord.events.get("onMessageBuilderLoad").listen((messages) => {
    ...
    messages.get("ot-ticket-forms:answers-message").workers.add(
        new api.ODWorker("ot-ticket-forms:answers-message", 0, async (instance, params, source, cancel) => {
            const { formInstanceId, sessionId, type, currentPageNumber, totalPages, currentPage } = params;
            ...
            if ( type === "initial" || type === "partial" ) {
                ...
                instance.addComponent(await opendiscord.builders.buttons.getSafe("ot-ticket-forms:delete-answers-button").build(source, { formInstanceId, sessionId }));
            } else {
