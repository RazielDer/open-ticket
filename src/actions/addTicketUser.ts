...
export const registerActions = async () => {
    ...
    opendiscord.actions.get("opendiscord:add-ticket-user").workers.add([
        new api.ODWorker("opendiscord:add-ticket-user",2,async (instance,params,source,cancel) => {
            ...
            if (channel.isThread()){
                try{
                    await (channel as discord.PrivateThreadChannel).members.add(data.id)
                }catch(err){
                    ...
                    opendiscord.log("Unable to add user to private-thread ticket before state mutation.","warning",[{key:"channelid",value:channel.id,hidden:true},{key:"userid",value:data.id,hidden:true}])
                    return cancel()
                }
            }
